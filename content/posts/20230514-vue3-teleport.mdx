---
title: 使用 Vue3 Teleport 製作對話框（Modal）
description: Vue3 Teleport 對話框
date: 2023-05-14
category: Vue
published: true
---

![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*GmILIQpmY39fENVOR-tGCg.png)
...
```html:ModalConfirm.vue
<template>
  <div class="absolute inset-0 overflow-hidden bg-black bg-opacity-50">
    <div class="mt-24 flex min-h-screen items-start justify-center text-center">
      <div
        class="w-64 rounded-md bg-white p-5 text-center text-black shadow-lg"
        role="dialog"
        aria-modal="true"
      >
        <h3>Do you confirm?</h3>
        <div class="flex justify-center gap-4 py-4 text-white">
          <!-- We will handle these emits later -->
          <button
            @click="$emit('close')"
            class="w-16 rounded-md border border-black bg-white py-2 text-black"
          >
            No
          </button>
          ...
          <button
            @click="$emit('close')"
            class="w-16 rounded-md border border-black bg-black py-2 text-white"
          >
            Yes
          </button>
        </div>
...
![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*iBUM71aTjPr9GkshIvIN5w.png)

## 破壞開啟 Modal 時的 UI

```html:App.vue showLinNumber{18-21}
<script setup lang="ts">
  import {ref} from 'vue'
  import ModalConfirm from '@/components/ModalConfirm.vue'
  const show = ref(false)
...
![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*6i9vBx1FYWFpRd8DB-qJyg.png)

來看一下 DOM 裡發生了什麼事!

![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*SIqtdrz3rJW9Aedpk07xJw.png)
...
![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*ZlrkTTK4BmL3W9cttV0SaA.png)

來看一下目前 DOM 的情況：
![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*ZCCABwrfL3vb7gsw-ZkXnA.png)
...
![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*XqQFaAH_GM0yf_7hPkwknA.png)
...
![](https://miro.medium.com/v2/resize:fit:720/format:webp/1*XLg9RtZ4301ZTBcSWAmFmw.png)
...
1. [Vue Teleport](https://vuejs.org/guide/built-ins/teleport.html)
2. [Vue Transition](https://vuejs.org/guide/built-ins/transition.html#css-based-transitions)
3. [Vue markraw](https://vuejs.org/api/reactivity-advanced.html#markraw)
4. [Vue dynamic components](https://vuejs.org/guide/essentials/component-basics.html#dynamic-components)
